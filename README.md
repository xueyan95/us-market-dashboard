...
| 飞书推送 | 飞书群自定义机器人 webhook | **FEISHU_WEBHOOK** |
...
| `FEISHU_WEBHOOK` | 飞书群机器人 webhook URL（见下） |
...
### 4. 查看看板
每次运行后在 Actions 的 run 详情底部 → Artifacts → 下载 `us-market-dashboard` → 解压得 `index.html`，浏览器打开即可（单文件、零外链、手机/电脑自适应、支持白天/夜间/跟随系统三态主题）。
